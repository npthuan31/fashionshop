...
$(function () {
    ...
    var message=$("#cartMessage").val();
    if(message)
    {
        $('.nav-cart-container').css('opacity','1').css('visibility','visible');
        setTimeout(function () {
            $('.nav-cart-container').css('opacity','').css('visibility','');
        },3000);
    }

    $('.minusCartItem').click(function () {
        var id=$(this).attr('id');
        if(Number($('#quantity_' + id).val()) > 1)
        {
            var newQuantity=Number($('#quantity_' + id).val()) - 1;
            $('#quantity_' + id).val(newQuantity);
            $('#cartEditForm_' + id).submit();
        }
    });

    $('.plusCartItem').click(function () {
        var id=$(this).attr('id');
        if(Number($('#quantity_' + id).val()) < 10)
        {
            var newQuantity=Number($('#quantity_' + id).val()) + 1;
            $('#quantity_' + id).val(newQuantity);
            $('#cartEditForm_' + id).submit();
